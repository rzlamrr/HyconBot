...
COMMANDS_HELP["devices"] = {
    ...
    "commands": {
        "user": {
            "device": "retrieve all supported device codenames",
            "device <codename>": "Get information about the device and builds with such a code name (if it is officially maintained)."
        }
    },
    "text": "This is the plugin where you can get data about supported device.",
    "help": True
}
...
@Client.on_message(filters.cmd('device'))
async def on_device_m(c: Client, m: Message):
    if len(m.text.split(" ")) >= 2:
        codename = m.text.split(" ")[1]
        ...
        infd = '**List of our currently supported devices:**\n'
        devices = ""
        for model in models:
            devices += model['codename'] + " "
        devices = [x for x in devices.split() if x]
        devices = sorted(devices, key=str.casefold)
        for depis in devices:
            infd += "\n- " + "`" + depis + "`"
        infd += '\n\nYou can get more information by using `/device codename`'
        await m.reply_text(infd, parse_mode="markdown")
...
async def get_device(u, model: Dict):
    ...
    for version in model['supported_versions']:
        maintainers.append(
            ...
            ')')
        versions.append(version['version_name'] +
...
def get_changelog(device: str, filename: str) -> str:
    response = requests.get(DEVICES_REPO +
                            f'/changelogs/{device}/{filename}.txt')
    data = response.text
    changelog = data
    return changelog
        ...
        if cq.from_user.id == builds['user_id']:
            ...
            if len(changelog) < 1200:
                ...
                download_keyboard.append(
                    ('📜 Changelog', f'get_changelog {id} {page}'))
            ...
            if not len(builds) == page:
                page_keyboard.append(
                    ('⬅️ Previous', f'get_builds {id} {page-2}'))
            if len(builds) > page:
